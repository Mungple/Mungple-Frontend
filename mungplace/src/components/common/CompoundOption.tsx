...
// 모달을 렌더링하는 메인 컴포넌트
function OptionMain({children, isVisible, hideOption, animationType = 'slide', ...props}: OptionMainProps) {
  ...
  return (
    <Modal visible={isVisible} transparent={true} animationType={animationType} onRequestClose={hideOption} {...props}>
...
function Background({children}: PropsWithChildren) {
  ...
  return <OptionBackground onTouchEnd={optionContext?.onClickOutSide}>{children}</OptionBackground>
}
...
function Button({children, isDanger = false, isChecked = false, ...props}: ButtonProps) {
  return (
    <StyledPressable {...props}>
      ...
      {isChecked && <Ionicons name="checkmark" size={20} color={colors.BLUE.DARKER} />}
    </StyledPressable>
  )
}
...
function CheckBox({children, icon, isChecked = false, ...props}: CheckBoxProps) {
  return (
    <StyledCheckBox {...props}>
      <Ionicons name={`checkmark-circle${isChecked ? '' : '-outline'}`} size={22} color={colors.BLUE.DARKER} />
      {icon}
...
function Filter({children, isSelected = false, ...props}: FilterProps) {
  return (
    <FilterContainer {...props}>
      ...
      <MaterialIcons name="keyboard-arrow-down" size={22} color={isSelected ? colors.BLUE.DARKER : colors.GRAY_300} />
    </FilterContainer>
  )
}
...
const FilterText = styled.Text<{isSelected: boolean}>`
  color: ${({isSelected}) => (isSelected ? colors.BLUE.DARKER : colors.GRAY_300)};
  font-size: 15px;
  font-weight: 500;
`
