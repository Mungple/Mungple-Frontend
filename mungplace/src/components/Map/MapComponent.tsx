  ...
  return (
    <Container>
      ...
      >
        ...
        {/* Heatmaps */}
        {showPersonalBlueZone && <Heatmap points={createHeatmapPoints(personalBlueZones)} />}
        {showGlobalBlueZone && <Heatmap points={createHeatmapPoints(globalBlueZones)} />}
        {showRedZone && <Heatmap points={createHeatmapPoints(redZones)} radius={50} opacity={0.6} />}
      ...
      <Animated.View style={{ transform: [{ translateY }], opacity, position: 'absolute', top: 0, right: 0 }}>
        ...
        <ButtonWithTextContainer top={120} right={20}>
          ...
          <CustomMapButton onPress={() => {}} iconName="settings" inValid={isDisabled} />
        </ButtonWithTextContainer>
      </Animated.View>

      <CustomMapButton onPress={handlePressUserLocation} iconName="locate" bottom={20} left={20} />
      <CustomMapButton onPress={() => {}} iconName="refresh" bottom={20} right={20} />
    </Container>
  );
};
