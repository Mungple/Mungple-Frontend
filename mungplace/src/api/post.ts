...
const getSearchPosts = async (page = 1, query: string): Promise<ResponsePost[]> => {
  const {data} = await axiosInstance.get(`/posts/my/search?query=${query}&page=${page}`)
  return data
}
...
export {createPost, deletePost, getCalendarPosts, getPost, getPosts, getSearchPosts, updatePost}
export type {CalendarPost, RequestCreatePost, RequestUpdatePost, ResponseCalendarPost, ResponsePost, ResponseSinglePost}
